var ModelsController = function(scene){
	controller = this;
	...
	this.loader = {
		...
		import : function(options){
			var type = options.url.split('.')[1];
			if(this.hasOwnProperty(type)){
				this.initializeLoad(type, options)
			}else{
				opts = [controller.manager]
				this[type] = ConstructorHelper(THREE[ this.indices[type] ], opts);
				this.initializeLoad(type, options)
			}
		},
		initializeLoad : function(type, options){
			this[type].load(options.url, function(model){
		    	model.traverse( function ( child ) {
			        if ( child instanceof THREE.Mesh ) {
			        	...
			        	if(options.barycentric){
			        		controller.setUpBarycentricCoordinates(child.geometry)
			        	}
			    ...
			    controller.modelsCache[options.name] = model;
			    controller.scene.add( model );
			}, controller.onProgress, controller.onError)
		}
	}
}
ModelsController.prototype.setUpBarycentricCoordinates = function(geometry) {
...
ModelsController.prototype.importModel = function(url, options){
	var parsedOptions = {
		url : url,
		barycentric : options['barycentric'] || false,
		transparent : options['transparent'] || false,
		material : options['material'] || new THREE.MeshLambertMaterial({ color: 'white'}),
		name : options['name'] || 'model'+this.modelsCache.length,
		vNormals : options['vertexNormals'] || false,
		fNormals : options['faceNormals'] || false,		
	}
	this.loader.import(parsedOptions)
}
ModelsController.prototype.managerProgress = function ( item, loaded, total ){
    // console.log( item, loaded, total );
};
ModelsController.prototype.onProgress = function ( xhr ) {
...
ModelsController.prototype.onError = function ( xhr ) {
};
